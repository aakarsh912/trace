...
type DeliverableRow = {
  key: string;
  id?: string;       // present = existing DB row; absent = new
  description: string;
  showHints: boolean;
  hints: string[];
};
...
export function EditActionModal({ actionId, projectId, projectName, loaneeName }: Props): JSX.Element {
  ...
  // Focus title once modal is open and data is loaded
  useEffect(() => {
    if (open && !fetching) setTimeout(() => titleRef.current?.focus(), 50);
  }, [open, fetching]);
  ...
  const handleOpen = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as ActionData & { error?: string };
      if (!res.ok) {
        setFetchError(data.error ?? "Failed to load action");
        setFetching(false);
        return;
      }
      setTitle(data.title);
      setDescription(data.description ?? "");
      setIfcCategory(data.ifcCategory);
      ...
      setDeliverableRows(
        data.deliverables.map((d) => ({
          ...
          showHints: !!(d.documentHints?.length),
          hints: d.documentHints ?? [],
        }))
      );
    } catch {
  ...
  function addDeliverable(): void {
    setDeliverableRows((rows) => [...rows, { key: crypto.randomUUID(), description: "", showHints: false, hints: [] }]);
  }
  ...
  function addHint(dKey: string): void {
    setDeliverableRows((rows) => rows.map((r) => r.key === dKey ? { ...r, hints: [...r.hints, ""] } : r));
  }
  function removeHint(dKey: string, hIdx: number): void {
    setDeliverableRows((rows) => rows.map((r) => r.key === dKey ? { ...r, hints: r.hints.filter((_, i) => i !== hIdx) } : r));
  }

  const canSave = !fetching && title.trim() && deliverableRows.length > 0 && deliverableRows.every((d) => d.description.trim()) && !saving;
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em" }}>Edit Action</div>
            ...
            {fetching ? (
              /* Loading skeleton */
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 48, color: "var(--fg-tertiary)", fontSize: 13 }}>
                Loading…
              </div>
            ) : fetchError ? (
              /* Fetch error */
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 48, gap: 12 }}>
                <p style={{ color: "var(--status-attention-fg)", fontSize: 13, margin: 0 }}>{fetchError}</p>
                <button onClick={() => void handleOpen()} style={{ fontSize: 13, color: "var(--fg-secondary)", background: "none", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "5px 12px", cursor: "pointer", fontFamily: "inherit" }}>
                  Retry
                </button>
              </div>
            ) : (
              <>
                  ...
                  <div style={{
                    ...
                    <Field label="Recommendation">
                      <textarea
                        ...
                        rows={5}
                        placeholder={`The full recommendation as it will appear to ${loaneeName}.`}
                  ...
                  <div style={{
                    ...
                    <MetaField label="IFC Category">
                      <select value={ifcCategory} onChange={(e) => setIfcCategory(e.target.value as IfcCategory)} style={selectStyle}>
                        {IFC_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                      </select>
                    </MetaField>
                    ...
                    <MetaField label="Suggested department" hint="Hint for the loanee only.">
                      <input
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 12, color: "var(--fg-tertiary)", marginRight: "auto" }}>
                    <kbd style={{ fontFamily: "inherit", fontSize: 11, background: "var(--bg-subtle)", border: "1px solid var(--border)", borderRadius: 3, padding: "1px 4px" }}>Esc</kbd> to close
                  </span>
                  {saveError && <span style={{ fontSize: 12, color: "var(--status-returned-fg)" }}>{saveError}</span>}
                  <button
                    onClick={closeModal}
                    disabled={saving}
                    style={{
...
function DeliverableItem({
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg)" }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 10px 10px 12px" }}>
        <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 11.5, color: "var(--fg-tertiary)", paddingTop: 9, flexShrink: 0, width: 18 }}>
          {letter})
        </span>
        <textarea
          ref={textareaRef}
          ...
          style={{ flex: 1, border: "none", background: "transparent", padding: "7px 4px", fontFamily: "inherit", fontSize: 13.5, color: "var(--fg)", resize: "none", outline: "none", lineHeight: 1.45, overflow: "hidden" }}
        />
        {canRemove && (
          <button type="button" onClick={onRemove} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--fg-tertiary)", padding: 6, borderRadius: 4, flexShrink: 0, marginTop: 2, display: "flex", alignItems: "center" }}>
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M4 4l8 8M12 4l-8 8" />
            </svg>
          </button>
        ...
        <div style={{ padding: "0 12px 10px 38px", display: "flex", flexDirection: "column", gap: 5 }}>
          {row.hints.map((hint, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <input value={hint} onChange={(e) => onHintChange(i, e.target.value)} placeholder="e.g. Waste Management Plan (PDF)" style={{ ...inputStyle, flex: 1 }} />
              <button type="button" onClick={() => onRemoveHint(i)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--fg-tertiary)", padding: 4, display: "flex", alignItems: "center" }}>
                <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M4 4l8 8M12 4l-8 8" />
                </svg>
              </button>
          ...
          <button type="button" onClick={onAddHint} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 0", background: "none", border: "none", fontSize: 12, color: "var(--fg-tertiary)", cursor: "pointer", fontFamily: "inherit" }}>
...
function MetaField({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }): JSX.Element {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--fg-tertiary)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
      {children}
